功能：编写stft.m，针对复数信号进行时频分析，和周期图法做笔记
目标：编写复数信号的逆stft
...
% % Axis Generating
% fax =(0:(nfft-1)) * fs./nfft;                        % Frequency axis setting
...
t=0:1/Fs:2;
%输入复信号
x=(cos(2*pi*50*t)+sin(2*pi*100*t)*j).*(t>=0&t<1)+(0.5*cos(2*pi*150*t)+sin(2*pi*250*t)*j).*(t>=1&t<2);
figure();
