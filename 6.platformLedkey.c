...
//本次测试参考了https://blog.csdn.net/zqixiao_09/article/details/50888795，从而实现了设备与驱动的分离
//dev.c中定义了设备的物理资源
//drv.c中获取该资源并完成match功能，然后通过probe和remove实现对设备的操作函数
//经测试，无论是先insmod dev还是drv，只要二者的name字段是相同的，都可以实现match，从而结合使用。

//本程序将key,led二者功能结合，在app中实现读取key值，当按键按下时led点亮，松开后led熄灭的功能。
//后期在此基础上，添加中断功能，设备树功能。
...
int main(int argc, char **argv)
{
	...
	fd = open("/dev/axigpio", O_RDWR);
	if (fd < 0)
	{
		printf("can't open axigpio!\n");
	}
